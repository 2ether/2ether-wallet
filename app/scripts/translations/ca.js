...
tx_FoundOnChain_3           : '**Si no veus cap error, la teva transacció s\'ha enviat amb èxit.** El teu ETH o Tokens són on els has enviat. Si no pots veure aquest ETH o Tokens a la teva cartera / compte de bescanvi, i han passat més de 24 hores des de l\'enviament, si us plau [contacta amb aquest servei](https://kb.wallet.2ether.com/diving-deeper/ethereum-list-of-support-and-communities.html). Envia *l\'enllaç* de la teva transacció si demana amablement que investiguin.',
...
TXFEE_Desc                  : 'La comissió per la TX es paga als miners per incloure la teva TX en un block. És el `límit de gas` * `preu del gas`. [Pots convertir GWEI -> ETH aquí](https://wallet.2ether.com/helpers.html)',
...
NODE_Warning                : 'El teu node ha de ser HTTPS per tal de poder-t\'hi connectar utilitzant wallet.2ether.com. Pots [descarregar-te el repo de MyEtherWallet i executar-lo localment](https://github.com/kvhnuke/etherwallet/releases/latest) per connectar-te a qualsevol node. O, obtén un certificat SSSL gratuït amb [LetsEncrypt](https://letsencrypt.org/)',
...
/* View Wallet Details */
VIEWWALLET_Subtitle         : 'Això et permet descarregar diferents versions de les claus privades i tornar a imprimir la teva cartera de paper. És possible que vulguis fer això per [importar el teu compte a Geth/Mist](http://ethereum.stackexchange.com/questions/465/how-to-import-a-plain-private-key-into-geth/). Si vols comprovar el teu saldo, recomanem utilitzar un explorador de la blockchain com [explorer.2ether.com](https://explorer.2ether.com). ',
...
ERROR_35                        :  '(error_36) L\'adreça introduïda no s\'ha comprovat mitjantçant checksum. <a href="https://kb.wallet.2ether.com/addresses/what-does-checksummed-mean.html" target="_blank" rel="noopener noreferrer"> Més informació</a> (Input address is not checksummed. <a href="https://kb.wallet.2ether.com/addresses/what-does-checksummed-mean.html" target="_blank" rel="noopener noreferrer">What does that mean?</a>)',
...
PARITY_InvalidGasLimit          :  "(parity-05) El gas proporcionat és més enllà del límit. (Supplied gas limit is beyond limit. Try lowering the gas limit to 21000. [Learn More.](https://kb.wallet.2ether.com/gas/what-is-gas-ethereum.html))",
...
PARITY_Old                      :  "(parity-07) El nonce de la transacció és massa baix. Proveu d'augmentar-lo. (There is already a transaction with this [nonce](https://kb.wallet.2ether.com/transactions/what-is-nonce.html). Try incrementing the nonce by pressing the Generate button again, or [replace the pending transaction](https://kb.wallet.2ether.com/transactions/check-status-of-ethereum-transaction.html).)",
