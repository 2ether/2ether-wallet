...
/* Onboarding */
ONBOARD_welcome_title           :  'wallet.2ether.com에 오신 것을 환영합니다.',
...
ONBOARD_blockchain_content__5   :  'wallet.2ether.com 혹은 [etherscan.io](https://etherscan.io)을 통해서 보는 잔액은 개인 시스템이 아닌 블록체인의 데이터를 보고 있는 것입니다.',
...
ONBOARD_whymew_content__6       :  '걱정스러워도 MEW를 사용하고자 한다면, [하드웨어 지갑을 구입하세요](https://kb.wallet.2ether.com/hardware-wallets/hardware-wallet-recommendations.html)! 키를 안전하게 보존해줍니다.',
...
ONBOARD_secure_1_content__3     :  'URL을 항상 확인하세요: `https://wallet.2ether.com`.',
...
ONBOARD_secure_3_content__6     :  '[더 많은 보안 팁!](https://kb.wallet.2ether.com/getting-started/protecting-yourself-and-your-funds.html)',
...
NAV_TxStatus                : 'TX 상태',
tx_Details                  : '트랜잭션 내역',
tx_Summary                  : '거래량이 높은 기간(예를 들어 ICO 기간중)의 트랜잭션은 몇 시간 동안 보류될 수 있습니다. 이 도구는 이러한 TX를 찾아 "취소"하거나 교체할 수 있는 기능을 제공합니다. **이는 일반적으로 수행할 수 있는 작업이 아닙니다. 이 도구를 의존하지 말아야 하며 TX풀이 가득 찼을 때만 작동합니다. [여기서 이 도구에 대해 읽어 보십시오.](https://kb.wallet.2ether.com/transactions/check-status-of-ethereum-transaction.html)**',
...
tx_FoundOnChain_3           : '**오류 메시지가 없다면 트랜잭션이 성공적으로 전송된 것입니다.** 이제 전송한 ETH 혹은 토큰은 전송된 곳에 있습니다. ETH 또는 토큰을 다른 지갑/거래소 계정에 입금된 것을 볼 수 없다면, 그리고 보낸 후 24시간이 지난 후라면, [관련 서비스에 문의하시기 바랍니다](https://kb.wallet.2ether.com/diving-deeper/ethereum-list-of-support-and-communities.html). 문의할 때 트랜잭션에 해당하는 *링크*를 보내어 친절하게 당신의 상황을 살펴보라고 부탁하세요.',
...
TXFEE_Desc                  : 'TX 수수료는 TX를 포함한 블록을 캔 채굴자에게 지불됩니다. 이는 `가스 한도` * `가스 가격` 입니다. [GWEI단위 -> ETH단위로 변환할 수 있습니다.](https://wallet.2ether.com/helpers.html)',
...
/* Footer */
FOOTER_1                    : '자유롭게 쓸 수 있는 무료 오픈소스 클라이언트 인터페이스로서, 이더리움 지갑 생성 등을 지원합니다. 이더리움 블록체인과 쉽고 안전하게 상호 작용하세요. 지갑을 열기 전에 URL(wallet.2ether.com)을 다시한번 확인하세요. ',
FOOTER_1b                   : '개발자 ',
...
NODE_Warning                : 'wallet.2ether.com을 통해 로컬 노드로 연결하기 위해서는 로컬 노드가 HTTPS를 지원해야 합니다. 이를 지원하지 않는 노드에 연결하려면 [MyEtherWallet 저장소를 다운로드하여 로컬로 실행](https://github.com/kvhnuke/etherwallet/releases/latest)하거나, 로컬 노드에 무료 SSL 인증서 [LetsEncrypt](https://letsencrypt.org/)를 설치하세요. ',
NODE_Name                   : '노드 명 ',
NODE_Port                   : '노드 포트 ',
...
/* View Wallet Details */
VIEWWALLET_Subtitle         : '다른 버전의 개인 키를 다운로드하거나 종이 지갑 정보를 다시 인쇄할 수 있습니다. [계정을 Geth/Mist로 가져오기](https://ethereum.stackexchange.com/questions/465/how-to-import-a-plain-private-key-into-geth/)를 진행할 때 필요합니다. 잔액을 확인하려면 [explorer.2ether.com](https://explorer.2ether.com)과 같은 블록체인 조회 서비스를 사용하는 것을 권장합니다. ',
...
ERROR_17                    : '(error_18) 잔액이 부족합니다. 가스 한도 * 가스 가격 + 보낼 수량 총합이 현 계정 잔액보다 부족합니다. 계정에 ETH를 보충하거나 "모든 잔액 전송" 버튼을 누르세요. 이것이 오류라고 생각한다면 생성 버튼을 다시 누르세요. 필요한 수량 (d+)과 실제 수량: (d+). [더 배우기](https://kb.wallet.2ether.com/transactions/transactions-not-showing-or-pending.html)',
...
ERROR_28                    : '(error_29) 지갑에 액세스하기 위해서는 **키스토어 파일과 비밀번호** (또는 개인키)가 필요합니다. 별도로 저장하여 백업해주세요. 만약 그것을 저장하지 않으면 지갑을 복원할 방법이 없습니다. 자세한 내용은 [도움말 페이지](https://wallet.2ether.com/#help)를 참고해주세요. ',
...
ERROR_35                    : '(error_36) 입력된 주소가 체크섬되어있지 않습니다. <a href="https://kb.wallet.2ether.com/addresses/what-does-checksummed-mean.html" target="_blank" rel="noopener noreferrer"> 더 보기</a>', // 35
...
HELP_Desc                   : '잘못된 부분이나 다른 질문이 있으신가요? [우리에게 연락해주세요](mailto : support@wallet.2ether.com), 당신의 질문에 답변하는 것 뿐만 아니라, 향후 사용자들에게 더 유용할 수 있도록 페이지를 업데이트할 것입니다. ',
HELP_Remind_Title           :  '몇 가지 알림 ',
HELP_Remind_Desc_1          : '**이더리움, wallet.2ether.com & MyEtherWallet CX, 그리고 우리가 사용하는 JavaScript 라이브러리는 활발하게 개발되고 있습니다. ** 우리가 철저하게 테스트를 진행하여 수만 개의 지갑이 전 세계 사람들에 의해 성공적으로 만들어졌지만 예상치 못한 일이 발생하여 ETH를 잃을 가능성이 항상 존재합니다. 항상 조심해주세요. 만약 무슨 일이 발생한다면, 유감스럽지만 **우리는 잃어버린 이더리움에 대한 책임을 지지 않습니다.** ',
HELP_Remind_Desc_2          : 'wallet.2ether.com과 MyEtherWallet CX는 웹 지갑이 아닙니다. 계정을 만들거나 우리에게 당신이 가진 이더리움을 주면 안 됩니다. 모든 데이터는 당신의 컴퓨터와 브라우저에 남아 있을 것입니다. 우리는 당신이 쉽게 당신의 정보를 만들고, 저장하고, 접근하고 블록체인과 상호 작용할 수 있도록 도울 것입니다. ',
...
HELP_3_Desc_3               : 'wallet.2ether.com에서 "지갑 정보 보기" 페이지로 이동합니다. ',
...
HELP_5_Title                : '5) wallet.2ether.com을 오프라인/로컬에서 사용할 수 있나요?  ',
HELP_5_Desc_1               : 'MyEtherWallet을 GitHub 서버가 아닌 본인의 컴퓨터에서 이용할 수 있습니다. 오프라인 상태에서 지갑을 생성할 수 있으며, "오프라인 트랜잭션" 페이지에서 트랜잭션을 전송할 수 있습니다. ',
HELP_5_Desc_7               : 'wallet.2ether.com은 이제 본인의 컴퓨터에서 구동됩니다. ',
HELP_5_Desc_8               : '아직 사용이 미숙할 때는, `index.html` 뿐만 아니라 웹사이트를 가동하는 전체 폴더를 수정하지 말아 주세요. ',
HELP_5_Desc_9               : '우리는 wallet.2ether.com을 주기적으로 업데이트하고 있으므로 당신이 정기적으로 저장소의 업데이트된 버전을 사용하는 것을 권장합니다. ',
...
HELP_8_Desc_2               : '만약 wallet.2ether.com이 다운된다면, 다른 대안 도구(geth 또는 Ethereum Wallet / Mist 등)를 사용해야 합니다. 이때 MyEtherWallet에서 이더리움을 "꺼내야" 할 필요는 없습니다. 이더리움은 wallet.2ether.com을 통해서 생성된 지갑 내에 존재합니다. ',
...
HELP_14_Title               : '14) 일부 사이트에서는 마우스의 움직임을 통해 무작위(씨드)로 개인 키를 생성합니다. wallet.2ether.com은 이 방식을 사용하지 않습니다. MyEtherWallet의 무작위 숫자 생성이 안전할까요? ',
HELP_14_Desc_1              : '대상을 움직이는 마우스는 영리하게 작동하며 우리는 사람들이 그것을 좋아하는 이유를 이해하지만, 실제 window.crypto는 당신의 마우스 조작보다 더 많은 엔트로피를 보장합니다. 마우스의 조작은 안전하지 못합니다. 우리와 다른 많은 암호 실험들은 window.crypto를 신뢰합니다. 게다가 wallet.2ether.com은 터치 장치에서도 이용될 수 있습니다. [화난 Reddit 이용자와 비탈릭의 마우스 조작 v. window.crypto에 대한 대화](https://www.reddit.com/r/ethereum/comments/2bilqg/note_there_is_a_paranoid_highsecurity_way_to/cj5sgrm) 및 [window.crypto w3 스펙](https://dvcs.w3.org/hg/webcrypto-api/raw-file/tip/spec/Overview.html#dfn-GloblCrypto)을 참조하세요. ',
...
HELP_16_Desc_1              : '[explorer.2ether.com](https://explorer.2ether.com)와 같은 블록체인 탐색기를 사용할 수 있습니다. 주소를 검색 창에 붙여넣으면 주소와 거래 내역이 표시됩니다. 예를 들어, 저희의 [기부 계정](https://etherscan.io/address/0xDECAF9CD2367cdbb726E904cD6397eDFcAe6068D)과 같은 식으로 보입니다. ',
...
HELP_SecCX_Desc_6           : '별명과 지갑 주소를 암호화하면 계정 잔액과 별명을 확인할 때마다 비밀번호를 입력해야 합니다. 보안이 걱정된다면 Chrome 확장 프로그램 대신 wallet.2ether.com을 이용해주세요. ',
...
HELP_Sec_Desc_4             : '이 URL을 확인해보세요. -- 이 사이트는 GitHub를 통해 소스가 공개되어 있으며 다음 링크에서 확인하실 수 있습니다. : [https://github.com/kvhnuke/etherwallet/tree/gh-pages](https://github.com/kvhnuke/etherwallet/tree/gh-pages) to [https://wallet.2ether.com](https://wallet.2ether.com). ',
